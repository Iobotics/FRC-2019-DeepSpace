...
public final class Main {
  static CvSource outputStream0;
  static CvSource outputStream1;

  static boolean buttonToggle;

  static NetworkTable table;
  static NetworkTableEntry buttonStatus;
  ...
  public static void main(String... args) {
    ...
    //buttonStatus = table.getEntry("table");
    ...
    if (cameras.size() >= 1) {
      VisionThread visionThread = new VisionThread(cameras.get(0),
        ...
        // do something with pipeline results
                outputStream0 = CameraServer.getInstance().putVideo("Camera0", 160, 120);
                outputStream1 = CameraServer.getInstance().putVideo("Camera1", 160, 120);
                cameras.get(0).setResolution(160, 120);
                cameras.get(1).setResolution(160, 120);

                CvSink cvSink0 = CameraServer.getInstance().getVideo(cameras.get(0));
                CvSink cvSink1 = CameraServer.getInstance().getVideo(cameras.get(1));
                ...
                cameras.get(0).setConnectionStrategy(VideoSource.ConnectionStrategy.kKeepOpen);
                cameras.get(1).setConnectionStrategy(VideoSource.ConnectionStrategy.kKeepOpen);
                ...
                while(!VisionThread.interrupted())
                {
                  ...
                  //buttonToggle = buttonStatus.getBoolean(false);

                  //if(buttonToggle)
                  //{
                    //sink.setSource(cameras.get(0));
                  //}
                  //else if(!buttonToggle)
                  //{
                    //sink.setSource(cameras.get(1));
                  //}
                }
